    ...
    public static function get_sales_analytics($start_date, $end_date, $compare = true)
    {
        ...
        foreach ($product_rows as $row) {
            ...
            $results['top_products'][] = array(
                'id' => $p_id,
                'name' => $row->name ? $row->name : 'منتج غير معروف',
        ...
        foreach ($customer_rows as $row) {
            ...
            if (empty($customer_name)) {
                $customer_name = $row->email ? $row->email : 'عميل زائر';
            }
        ...
        $funnel_data = array();
        $steps = array(
            'view_session' => 'زيارة الموقع',
            'add_to_cart' => 'إضافة للسلة',
            'begin_checkout' => 'بدء الدفع',
            'add_shipping_info' => 'معلومات الشحن',
            'add_payment_info' => 'معلومات الدفع',
            'purchase' => 'شراء مكتمل',
        );
        ...
        $eg_states = array(
            'C' => array('code' => 'EG-C', 'name' => 'القاهرة'),
            'KH' => array('code' => 'EG-C', 'name' => 'القاهرة'),
            'ALX' => array('code' => 'EG-ALX', 'name' => 'الإسكندرية'),
            'GZ' => array('code' => 'EG-GZ', 'name' => 'الجيزة'),
            'QAL' => array('code' => 'EG-QAL', 'name' => 'القليوبية'),
            'DK' => array('code' => 'EG-DK', 'name' => 'الدقهلية'),
            'BH' => array('code' => 'EG-BH', 'name' => 'البحيرة'),
            'FYM' => array('code' => 'EG-FYM', 'name' => 'الفيوم'),
            'GH' => array('code' => 'EG-GH', 'name' => 'الغربية'),
            'KB' => array('code' => 'EG-KB', 'name' => 'المنوفية'),
            'IS' => array('code' => 'EG-IS', 'name' => 'الإسماعلية'),
            'SUZ' => array('code' => 'EG-SUZ', 'name' => 'السويس'),
            'PTS' => array('code' => 'EG-PTS', 'name' => 'بورسعيد'),
            'ASW' => array('code' => 'EG-ASW', 'name' => 'أسوان'),
            'AST' => array('code' => 'EG-AST', 'name' => 'أسيوط'),
            'BNS' => array('code' => 'EG-BNS', 'name' => 'بني سويف'),
            'DA' => array('code' => 'EG-DA', 'name' => 'دمياط'),
            'KSH' => array('code' => 'EG-KSH', 'name' => 'كفر الشيخ'),
            'LX' => array('code' => 'EG-LX', 'name' => 'الأقصر'),
            'MN' => array('code' => 'EG-MN', 'name' => 'المنيا'),
            'MS' => array('code' => 'EG-MS', 'name' => 'مطروح'),
            'NS' => array('code' => 'EG-NS', 'name' => 'شمال سيناء'),
            'SHG' => array('code' => 'EG-SHG', 'name' => 'سوهاج'),
            'SHR' => array('code' => 'EG-SHR', 'name' => 'الشرقية'),
            'SS' => array('code' => 'EG-SS', 'name' => 'جنوب سيناء'),
            'WAD' => array('code' => 'EG-WAD', 'name' => 'الوادي الجديد'),
            'BA' => array('code' => 'EG-BA', 'name' => 'البحر الأحمر'),
            'QNA' => array('code' => 'EG-QNA', 'name' => 'قنا'),
        );
        ...
        foreach ($geo_rows as $row) {
            ...
            if (isset($eg_states[$state_code])) {
                $st_info = $eg_states[$state_code];
                $state_key = $st_info['code'];
                $state_name = $st_info['name'];
                if (isset($geo_data[$state_key])) {
                    $geo_data[$state_key]['sales'] += (float) $row->total_sales;
                    $geo_data[$state_key]['orders'] += (int) $row->order_count;
                } else {
                    $geo_data[$state_key] = array(
                        'code' => $state_key,
                        'name' => $state_name,
        ...
        // 9.11. استعلام مصفوفة احتفاظ العملاء بالمجموعات الشهرية (Monthly Cohort Retention Heatmap)
        $six_months_start = date('Y-m-01 00:00:00', strtotime('-5 months'));
        if ($is_hpos) {
            $cohort_query = $wpdb->prepare(
                "SELECT 
                    cohort_month,
                    ...
                    COUNT(DISTINCT active_cust) as active_customers
                 FROM (
                    SELECT 
                        -- إصلاح: wc_orders لا تحتوي على عمود cust_id، يجب حساب المعرّف بالـ CASE
                        CASE WHEN cust_orders.customer_id > 0 THEN CAST(cust_orders.customer_id AS CHAR) ELSE cust_orders.billing_email END as cust_id,
                        first_orders.first_month as cohort_month,
                        DATE_FORMAT(cust_orders.date_created_gmt, '%%Y-%%m-01') as order_month,
                        CASE WHEN cust_orders.customer_id > 0 THEN CAST(cust_orders.customer_id AS CHAR) ELSE cust_orders.billing_email END as active_cust
                    FROM {$wpdb->prefix}wc_orders cust_orders
                    JOIN (
                        SELECT 
                            CASE WHEN customer_id > 0 THEN CAST(customer_id AS CHAR) ELSE billing_email END as cust_id,
                            DATE_FORMAT(MIN(date_created_gmt), '%%Y-%%m-01') as first_month
                        FROM {$wpdb->prefix}wc_orders
                        WHERE type = 'shop_order' AND status IN ({$statuses_in})
                          AND (customer_id > 0 OR (billing_email IS NOT NULL AND billing_email != ''))
                        GROUP BY cust_id
                    ) first_orders ON (CASE WHEN cust_orders.customer_id > 0 THEN CAST(cust_orders.customer_id AS CHAR) ELSE cust_orders.billing_email END) = first_orders.cust_id
                    WHERE cust_orders.type = 'shop_order'
                      AND cust_orders.status IN ({$statuses_in})
                      AND first_orders.first_month >= %s
                      AND (cust_orders.customer_id > 0 OR (cust_orders.billing_email IS NOT NULL AND cust_orders.billing_email != ''))
                 ) as cohort_data
                 GROUP BY cohort_month, order_month, month_offset
                ...
                "SELECT 
                 ...
                 FROM (
                    SELECT 
                        cust_orders.cust_id,
                        first_orders.first_month as cohort_month,
                        DATE_FORMAT(cust_orders.post_date, '%%Y-%%m-01') as order_month,
                        cust_orders.cust_id as active_cust
                    FROM (
                        SELECT 
                            o.ID,
                            o.post_date,
                        ...
                        LEFT JOIN {$wpdb->postmeta} pm_cust ON o.ID = pm_cust.post_id AND pm_cust.meta_key = '_customer_user'
                        LEFT JOIN {$wpdb->postmeta} pm_email ON o.ID = pm_email.post_id AND pm_email.meta_key = '_billing_email'
                        WHERE o.post_type = 'shop_order' AND o.post_status IN ({$statuses_in})
                    ) cust_orders
                    JOIN (
                        SELECT 
                            cust_id,
                            DATE_FORMAT(MIN(post_date), '%%Y-%%m-01') as first_month
                        FROM (
                            SELECT 
                                o.post_date,
                                CASE 
                        ...
                        GROUP BY cust_id
                    ) first_orders ON cust_orders.cust_id = first_orders.cust_id
                    WHERE first_orders.first_month >= %s
                      AND cust_orders.cust_id IS NOT NULL AND cust_orders.cust_id != ''
                 ) as cohort_data
                 GROUP BY cohort_month, order_month, month_offset
                 ORDER BY cohort_month ASC, month_offset ASC",
                $six_months_start
        ...
        $cohort_rows = $wpdb->get_results($cohort_query);
        $cohort_map = array();

        if ($cohort_rows) {
            foreach ($cohort_rows as $c_row) {
                $c_month = $c_row->cohort_month;
                $c_offset = (int) $c_row->month_offset;
                if ($c_offset < 0 || $c_offset > 5)
                    continue;

                if (!isset($cohort_map[$c_month])) {
                    $c_time = strtotime($c_month);
                    $formatted_cohort = date_i18n('F Y', $c_time);
                    $cohort_map[$c_month] = array(
                        'cohort_name' => $formatted_cohort,
                        'total_size' => (int) $c_row->total_customers,
                        'retention' => array_fill(0, 6, null),
                    );
                }

                $tot_size = $cohort_map[$c_month]['total_size'];
                $act_cust = (int) $c_row->active_customers;
                $pct = $tot_size > 0 ? round(($act_cust / $tot_size) * 100, 1) : 0;
                $cohort_map[$c_month]['retention'][$c_offset] = $pct;
            }
        }

        $results['cohort_retention'] = array(
            'headers' => array('شهر 0', 'شهر +1', 'شهر +2', 'شهر +3', 'شهر +4', 'شهر +5'),
            'cohorts' => array_values($cohort_map),
        );
        ...
        foreach ($payment_rows as $row) {
            ...
            $title = $row->method_title ? $row->method_title : 'وسيلة دفع غير محددة';
            $orders = (int) $row->order_count;
        ...
        $rfm_meta = array(
            'champions' => array('label' => 'أبطال الشراء', 'icon' => '👑', 'color' => '#10b981'),
            'loyal' => array('label' => 'عملاء مخلصون', 'icon' => '💎', 'color' => '#6366f1'),
            'promising' => array('label' => 'عملاء واعدون', 'icon' => '🌟', 'color' => '#3b82f6'),
            'at_risk' => array('label' => 'عملاء معرضون للخطر', 'icon' => '⚠️', 'color' => '#f59e0b'),
            'lost' => array('label' => 'عملاء نائمون', 'icon' => '😴', 'color' => '#ef4444'),
            'new_customers' => array('label' => 'عملاء جدد', 'icon' => '✨', 'color' => '#8b5cf6'),
        );
        ...
        foreach ($affinity_rows as $row) {
            ...
            if (isset($affinity_merged[$pair_key])) {
                ...
                $name_a = $eff_a !== $id_a ? get_the_title($eff_a) : ($row->product_a_name ?: 'منتج غير محدد');
                $name_b = $eff_b !== $id_b ? get_the_title($eff_b) : ($row->product_b_name ?: 'منتج غير محدد');
        ...
        usort($affinity_merged, function ($a, $b) {
